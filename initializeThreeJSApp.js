...
export const initializeApp = (container) => {
  ...
  function createOrbitEllipse(
    ...
    for (let t = 0; t < numPoints; t++) {
      ...
      const x =
        xOrbitalPlane *
          (Math.cos(Ω) * Math.cos(ω) -
            Math.sin(Ω) * Math.sin(ω) * Math.cos(i)) -
        yOrbitalPlane *
      ...
      const y =
        xOrbitalPlane *
          (Math.sin(Ω) * Math.cos(ω) +
            Math.cos(Ω) * Math.sin(ω) * Math.cos(i)) +
        yOrbitalPlane *
  ...
  function addOrbitsToScene() {
    ...
    const mercuryOrbitPoints = createOrbitEllipse(
      ...
      252.2503235 // Number of points for smooth orbit
    );
    ...
    const venusOrbitPoints = createOrbitEllipse(
      ...
      181.9790995 // Number of points for smooth orbit
    );
